...
;; Add error handling for missing items in `bid-item`, `end-auction`, and `buy-item`

(define-public (bid-item (id uint) (amount uint))
    (let ((item-info (map-get? items { id: id })))
        (match item-info
            item
            (if (and (is-eq (get status item) u0) (> amount (get highest-bid item)) (>= (get end-time item) (block-height)))
                (begin
